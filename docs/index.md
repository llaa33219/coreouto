...
A minimal, extensible Python agent library. An agent is called with a message, runs an internal loop where it can use tools, and returns its response when the model wraps its final answer in `<finish>...</finish>` tags. Everything else is an opt-in extension.
...
**Minimalism.** coreouto implements only what an agent system needs to function. There is one loop, one termination protocol (`<finish>` tag), one way to wire things together. If a feature can live outside the library, it does. The core stays small so you can read the whole thing in an afternoon.
