...
An agent library needs to do exactly one thing well: run a loop where an LLM calls tools and eventually returns a result. coreouto's core is that loop, the **one termination rule** that ends it (a response with text content and no tool calls means the model is done; the assistant text is the final answer), and the data structures that flow through it. That's it.
